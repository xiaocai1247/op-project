package com.xiaocai.wechat.service.impl;
...
import com.xiaocai.wechat.mapper.MenuMapper;
import com.xiaocai.wechat.service.MenuService;
...
//                .collect(Collectors.toList());
        ...
        for (Menu oneMenu : oneMenuList) {
            ...
            //4 获取每个一级菜单里面的二级菜单 id 和  parent_id 比较
            //一级菜单id 和 其他菜单 parent_id
            List<Menu> twoMenuList = menuList.stream()
                    .filter(menu -> menu.getParentId().longValue() == oneMenu.getId())
                    .collect(Collectors.toList());
            //5 把一级菜单里面所有二级菜单获取到，封装一级菜单children集合里
